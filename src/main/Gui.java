...
public class Gui {
	...
	private JButton button;//play
	private JButton button_1;//step
	private JButton button_2;//pause
	private JButton button_3;//open
	...
	private void initialize() {
		...
		String[] opcoes = {"Estatico","Um bit","Dois Bits"};
	    comboBox = new JComboBox(opcoes);
		comboBox.setBounds(544, 11, 94, 20);
		frame.getContentPane().add(comboBox);
		
		JLabel lblCoisaFutura = new JLabel("Preditor");
	...
	private void addActionListeners() {
		...
		button_1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(!p.isFinished()&&!runner.isRunning()){
					runner.runStep();
				}
			}
		});
	...
	public void updateInfos() {
		...
		setCpi(completed>0 ? new Float(new Float(clocks)/new Float(completed)) : null);
		numInstConcluidas.setText(completed.toString());
		updateReorder();
		if(endereco1.getText()!=null&&p.getDataMemory().getLastAddress()!=null){
			String lastAddress = endereco1.getText();
			if(!lastAddress.equals(p.getDataMemory().getLastAddress().toString()))
	...
	public void resetAll(Processor p){
		setProcessor(p);
		runner = new Runner(this, p);
		runner.execute();
		updateInfos();
